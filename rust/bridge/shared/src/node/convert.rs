...
pub(crate) struct AssumedImmutableBuffer<'a> {
    buffer: &'a [u8],
    hash: u64,
}
...
pub(crate) struct PersistentBoxedValue<T: Send + 'static> {
    owner: Root<JsObject>,
    value_ptr: *const T,
}

impl<T: Send + 'static> PersistentBoxedValue<T> {
    pub(crate) fn new<'a>(
        cx: &mut impl Context<'a>,
...
impl<T: Send + 'static> Deref for PersistentBoxedValue<T> {
    type Target = T;
    fn deref(&self) -> &T {
...
unsafe impl<T: Send + 'static> Send for PersistentBoxedValue<T> {}

impl<T: Send + 'static> Finalize for PersistentBoxedValue<T> {
    fn finalize<'a, C: Context<'a>>(self, cx: &mut C) {
        self.owner.finalize(cx)
    }
}
